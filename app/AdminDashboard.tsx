...
export function AdminDashboard({ apiBase = "", authPassword = "", onUnauthorized }: { apiBase?: string; authPassword?: string; onUnauthorized?: () => void } = {}) {
  ...
  return (
    <main className="app-shell">
      ...
      {selected && <CustomerPanel customer={selected} tags={data.tags} onClose={() => setSelected(null)} onSave={async (tagIds) => { await action({ action: "set-customer-tags", customerId: selected.id, tagIds }); setSelected(null); }} />}
...
function ScenarioMessageEditor({ message, allTags, onClose, onSave }: { message: ScenarioMessage; allTags: Tag[]; onClose: () => void; onSave: (message: string, tagIds: number[], newTagNames: string[]) => Promise<void> }) {
  const [text, setText] = useState(message.message);
  const [selectedTagIds, setSelectedTagIds] = useState(message.tags.map((tag) => tag.id));
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(""); try { await onSave(text, selectedTagIds, newTagNames); } catch (err) { setError(err instanceof Error ? err.message : "Не удалось сохранить сообщение"); setBusy(false); } }
  return <div className="overlay composer-overlay"><form className="composer scenario-editor" onSubmit={submit}><header><div><span className="eyebrow">Сообщение сценария</span><h2>{message.title}</h2></div><button type="button" className="close-button static" onClick={onClose}>×</button></header><div className="composer-body"><label className="field"><span>Текст сообщения</span><textarea required rows={12} maxLength={4096} value={text} onChange={(event) => setText(event.target.value)}/><small>{text.length} / 4096</small></label><div className="scenario-tag-editor"><strong>Теги после сообщения</strong><p>Нажмите на тег, чтобы добавить или убрать его.</p><div className="tag-choice">{allTags.map((tag) => { const selected = selectedTagIds.includes(tag.id); return <button type="button" className={`tag tag-${tag.color}${selected ? " selected" : ""}`} aria-pressed={selected} key={tag.id} onClick={() => setSelectedTagIds((current) => selected ? current.filter((id) => id !== tag.id) : [...current, tag.id])}>{selected ? "✓" : "＋"} {tag.name}</button>; })}</div><div className="new-tag-row"><input maxLength={60} placeholder="Название нового тега" value={newTag} onChange={(event) => setNewTag(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addTag(); } }}/><button type="button" className="secondary-button" onClick={addTag}>Добавить тег</button></div>{newTagNames.length > 0 && <div className="tag-choice pending-tags">{newTagNames.map((name) => <button type="button" className="tag tag-violet selected" key={name} onClick={() => setNewTagNames((current) => current.filter((item) => item !== name))}>× {name}</button>)}</div>}</div><p className="editor-hint">Выбранные теги будут выдаваться клиенту после отправки сообщения. Удаление здесь не снимает тег с клиентов, которые уже его получили.</p><p className="editor-hint">Можно использовать форматирование Telegram: &lt;b&gt;жирный&lt;/b&gt;, &lt;i&gt;курсив&lt;/i&gt; и &lt;a href=&quot;ссылка&quot;&gt;текст ссылки&lt;/a&gt;. Кнопки сценария сохранятся автоматически.</p>{error && <p className="form-error">{error}</p>}</div><footer><button type="button" className="secondary-button" onClick={onClose}>Отмена</button><button className="primary-button" disabled={busy || !text.trim()}>{busy ? "Сохраняем…" : "Сохранить сообщение"}</button></footer></form></div>;
}
...
function Composer({ tags, campaign, onClose, onSave }: { tags: Tag[]; campaign: Campaign | null; onClose: () => void; onSave: (payload: Record<string, unknown>) => Promise<void> }) {
  ...
  function insertMarkup(opening: string, closing: string, placeholder: string) {
    const field = messageRef.current; const start = field?.selectionStart ?? message.length; const end = field?.selectionEnd ?? start;
    const content = message.slice(start, end) || placeholder; const next = `${message.slice(0, start)}${opening}${content}${closing}${message.slice(end)}`;
    setMessage(next);
    window.setTimeout(() => { field?.focus(); field?.setSelectionRange(start + opening.length, start + opening.length + content.length); });
  }
  function insertLink() {
    const field = messageRef.current; const start = field?.selectionStart ?? message.length; const end = field?.selectionEnd ?? start;
    const selectedText = message.slice(start, end); const linkText = selectedText || window.prompt("Какой текст сделать ссылкой?", "Перейти по ссылке")?.trim();
    if (!linkText) return;
    const href = window.prompt("Вставьте ссылку", "https://")?.trim();
    if (!href) return;
    try {
      const url = new URL(href);
      if (!['http:', 'https:'].includes(url.protocol)) throw new Error();
    } catch {
      setError("Введите полную ссылку, которая начинается с https://");
      return;
    }
    setError("");
    const safeHref = href.replaceAll("&", "&amp;").replaceAll('"', "&quot;");
    const next = `${message.slice(0, start)}<a href="${safeHref}">${linkText}</a>${message.slice(end)}`;
    setMessage(next);
    window.setTimeout(() => { field?.focus(); field?.setSelectionRange(start, start + next.length - message.length); });
  }
